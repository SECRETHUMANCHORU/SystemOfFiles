...
function buildAPI(globalOptions, html, token, jar) {
  ...
  logger(`[ DATABASE ] Logged in as ${userID}`);
  ...
  if (token == 'NONE') 
    ...
    logger(`[ DATABASE ] Cant get access_token`);
  if (oldFBMQTTMatch) {
    irisSeqID = oldFBMQTTMatch[1];
    mqttEndpoint = oldFBMQTTMatch[2];
    region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
    logger(`[ DATABASE ] » Account's message region: ${region}`);
  } else {
    ...
    if (newFBMQTTMatch) {
      ...
      if (legacyFBMQTTMatch) {
        ...
        region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
      logger(`[ DATABASE ] » Your account has been disconnected. Please check your Facebook account to see if anything has happened`);
    ...
    return utils
      ...
      .then(function(res) {
        ...
        if (headers.location.includes('.com/checkpoint/')) {
          ...
          logger(`[ LOGIN ] » Please check your Facebook and click 'This was me" as you have login approvals turned on.`);
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
  ...
  mainPromise
    .then(function() {
      logger(`[ DATABASE ] » Done logging in`); return callback(null, api);
    })
    .catch(function(e) {
